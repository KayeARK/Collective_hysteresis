...
save(results_up, results_down, light_levels, file = "Code/gamma_individual_b5_results.RData")
...
y_threshold_lower <- 0.5  # Below this = Lower branch
y_threshold_upper <- 0.5  # Above this = Upper branch
...
for (kernel_name in names(kernels)) {
  up_data <- plot_data[plot_data$kernel == kernel_name & plot_data$direction == "Upward", ]
  ...
  down_data <- plot_data[plot_data$kernel == kernel_name & plot_data$direction == "Downward", ]
...
p <- ggplot() +
  ...
  # Add arrows on steepest parts
  geom_segment(data = arrow_data,
               aes(x = x, y = y, xend = xend, yend = yend, color = kernel),
  ...
  scale_color_manual(values = c(
    ...
    "Gaussian" = brewer.pal(3, "Dark2")[2],
    "Exponential" = brewer.pal(3, "Dark2")[3]
  )) +
...
ggsave(
  filename = "hysteresis_individual_gamma_thresholds_beta5.pdf",
  plot = p,
  width = 7,
  height = 7,
  units = "in",
...
cat("Plot saved as 'hysteresis_individual_gamma_thresholds.pdf'\n")
