...
# -------------------------------
alpha <- 1
L_max <- 1  # Increased to allow curves to extend naturally
...
compute_fold_branches <- function(dist, params,
  ...
  curve <- data.frame(
    ...
    filter(L >= -2, L <= L_max, beta >= 0, beta <= beta_max)  # Allow negative L but not extremely negative
...
ggsave("Vary distributions/analytic_number_of_fps_vary_params_beta.pdf", width = 7, height = 7)
...
z_range <- seq(0, 1, length.out = 1000)

# Generate gamma PDF data for each parameter set
gamma_data <- data.frame(
  z = rep(z_range, 6),
  density = c(
    dbeta(z_range, 2, 10),      # branches1
    dbeta(z_range, 5, 10),  # branches2  
    dbeta(z_range, 10, 10),      # branches3
    dbeta(z_range, 20, 10),      # branches4
    dbeta(z_range, 50, 10),    # branches5
    dbeta(z_range, 100, 10)   # branches6
  ),
...
p_gamma <- ggplot(gamma_data, aes(x = z, y = density, color = factor(color_index))) +
  ...
  labs(x = expression("Threshold, " * theta), 
       y = expression("Probability density, Beta(" * a * "," * b * ")")) +
  theme_minimal(base_size = 13) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
  ...
  coord_cartesian(xlim = c(0, 1), ylim = c(0, max(gamma_data$density) * 1.05))+
  scale_x_continuous(expand = c(0, 0.02)) +
  scale_y_continuous(expand = c(0, 0))

print(p_gamma)
ggsave("Vary distributions/beta_distributions_vary_params.pdf", plot = p_gamma, width = 7, height = 7)