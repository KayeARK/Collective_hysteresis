...
branches1 <- compute_fold_branches(1.2, 3.2/1.2, alpha, L_max, beta_max)
...
# Compute branches using the function
branches2 <- compute_fold_branches(1.5, 3.2/1.5, alpha, L_max, beta_max)
...
# Compute branches using the function
branches3 <- compute_fold_branches(2, 3.2/2, alpha, L_max, beta_max)
...
# Compute branches using the function
branches4 <- compute_fold_branches(4, 3.2/4, alpha, L_max, beta_max)
...
# Compute branches using the function
branches5 <- compute_fold_branches(8, 3.2/8, alpha, L_max, beta_max)
...
# Compute branches using the function
branches6 <- compute_fold_branches(16, 3.2/16, alpha, L_max, beta_max)
...
shape_values <- c(1.2,1.5,2,4, 8, 16)
scale_values <- 3.2/shape_values


ggplot() +
  
  # fold curves with different rainbow shades
  geom_path(data = lower_branch1, aes(L, beta), colour = rainbow_colors[1], linewidth = 1.2) +  # Dark red
  geom_path(data = upper_branch1, aes(L, beta), colour = rainbow_colors[1], linewidth = 1.2) +
  geom_path(data = lower_branch2, aes(L, beta), colour = rainbow_colors[2], linewidth = 1.2) +  # Orange red
  geom_path(data = upper_branch2, aes(L, beta), colour = rainbow_colors[2], linewidth = 1.2) +
  geom_path(data = lower_branch3, aes(L, beta), colour = rainbow_colors[3], linewidth = 1.2) +  # Gold
  geom_path(data = upper_branch3, aes(L, beta), colour = rainbow_colors[3], linewidth = 1.2) +
  geom_path(data = lower_branch4, aes(L, beta), colour = rainbow_colors[4], linewidth = 1.2) +  # Lime green
  geom_path(data = upper_branch4, aes(L, beta), colour = rainbow_colors[4], linewidth = 1.2) +
  geom_path(data = lower_branch5, aes(L, beta), colour = rainbow_colors[5], linewidth = 1.2) +  # Dodger blue
  geom_path(data = upper_branch5, aes(L, beta), colour = rainbow_colors[5], linewidth = 1.2) +
  geom_path(data = lower_branch6, aes(L, beta), colour = rainbow_colors[6], linewidth = 1.2) +  # Dark orchid
...
ggsave("Vary parameters/analytic_number_of_fps_vary_params_mean.pdf", width = 7, height = 7)
...
gamma_data <- data.frame(
  z = rep(z_range, 6),
  density = c(
    dgamma(z_range, shape = 1.2, scale = 3.2/1.2),      # branches1
    dgamma(z_range, shape = 1.5, scale = 3.2/1.5),  # branches2  
    dgamma(z_range, shape = 2, scale = 3.2/2),      # branches3
    dgamma(z_range, shape = 4, scale = 3.2/4),      # branches4
    dgamma(z_range, shape = 8, scale = 3.2/8),    # branches5
    dgamma(z_range, shape = 16, scale = 3.2/16)   # branches6
  ),
  group = rep(c("k=4.0, θ=0.8", "k=1.5, θ=2.1", "k=2.0, θ=1.6", 
                "k=8.0, θ=0.4", "k=16, θ=0.2", "k=1.2, θ=2.7"), each = length(z_range)),
  color_index = rep(c(1, 2, 3, 4, 5, 6), each = length(z_range))
)

# Create data for gamma distribution labels
gamma_labels <- data.frame(
  x = c(max(0, (4-1)*3.2/4) + 0.3,
        max(0, (1.5-1)*3.2/1.5) + 0.3,
        max(0, (2-1)*3.2/2) + 0.3,
        max(0, (8-1)*3.2/8) + 0.3,
        max(0, (16-1)*3.2/16) + 0.3,
        max(0, (1.2-1)*3.2/1.2) + 0.3),
  y = c(dgamma(max(0.01, (4-1)*3.2/4), shape = 4, scale = 3.2/4) + 0.05,
        dgamma(max(0.01, (1.5-1)*3.2/1.5), shape = 1.5, scale = 3.2/1.5) + 0.1,
        dgamma(max(0.01, (2-1)*3.2/2), shape = 2, scale = 3.2/2) + 0.05,
        dgamma(max(0.01, (8-1)*3.2/8), shape = 8, scale = 3.2/8) + 0.2,
        dgamma(max(0.01, (16-1)*3.2/16), shape = 16, scale = 3.2/16) + 0.5,
        dgamma(max(0.01, (1.2-1)*3.2/1.2), shape = 1.2, scale = 3.2/1.2) + 0.15),
  label = c("atop(italic(k)==4.0, theta==0.8)",
            "atop(italic(k)==1.5, theta==2.1)",
            "atop(italic(k)==2.0, theta==1.6)",
            "atop(italic(k)==8.0, theta==0.4)",
            "atop(italic(k)==16, theta==0.2)",
            "atop(italic(k)==1.2, theta==2.7)"),
  color = rainbow_colors
)

# Create the gamma distribution plot with rainbow colors and legend
...
print(p_gamma)
ggsave("Vary parameters/gamma_distributions_vary_params_mean.pdf", plot = p_gamma, width = 7, height = 7)