...
branches1 <- compute_fold_branches(4, 0.2, alpha, L_max, beta_max)
...
# Compute branches using the function
branches2 <- compute_fold_branches(4, 0.4, alpha, L_max, beta_max)
...
# Compute branches using the function
branches3 <- compute_fold_branches(4, 0.6, alpha, L_max, beta_max)
...
# Compute branches using the function
branches5 <- compute_fold_branches(4, 1, alpha, L_max, beta_max)
...
# Compute branches using the function
branches6 <- compute_fold_branches(4, 1.2, alpha, L_max, beta_max)
...
ggsave("Vary parameters/analytic_number_of_fps_vary_params_scale.pdf", width = 7, height = 7)
...
gamma_data <- data.frame(
  z = rep(z_range, 6),
  density = c(
    dgamma(z_range, shape = 4, scale = 0.2),      # branches1
    dgamma(z_range, shape = 4, scale = 0.4),  # branches2  
    dgamma(z_range, shape = 4, scale = 0.6),      # branches3
    dgamma(z_range, shape = 4, scale = 0.8),      # branches4
    dgamma(z_range, shape = 4, scale = 1),    # branches5
    dgamma(z_range, shape = 4, scale = 1.2)   # branches6
  ),
...
gamma_labels <- data.frame(
  x = c(max(0, (4-1)*0.2) + 0.3,
        max(0, (4-1)*0.4) + 0.3,
        max(0, (4-1)*0.6) + 0.3,
        max(0, (4-1)*0.8) + 0.3,
        max(0, (4-1)*1.0) + 0.3,
        max(0, (4-1)*1.2) + 0.3),
  y = c(dgamma(max(0.01, (4-1)*0.2), shape = 4, scale = 0.2) + 0.05,
        dgamma(max(0.01, (4-1)*0.4), shape = 4, scale = 0.4) + 0.05,
        dgamma(max(0.01, (4-1)*0.6), shape = 4, scale = 0.6) + 0.05,
        dgamma(max(0.01, (4-1)*0.8), shape = 4, scale = 0.8) + 0.05,
        dgamma(max(0.01, (4-1)*1.0), shape = 4, scale = 1.0) + 0.05,
        dgamma(max(0.01, (4-1)*1.2), shape = 4, scale = 1.2) + 0.05),
  label = c("atop(italic(k)==4.0, theta==0.2)",
            "atop(italic(k)==4.0, theta==0.4)",
            "atop(italic(k)==4.0, theta==0.6)",
            "atop(italic(k)==4.0, theta==0.8)",
            "atop(italic(k)==4.0, theta==1.0)",
            "atop(italic(k)==4.0, theta==1.2)"),
...
p_gamma <- ggplot(gamma_data, aes(x = z, y = density, color = factor(color_index))) +
  ...
  # Add labels at the mode (maximum) of each distribution using geom_text with parsing
  # geom_text(data = gamma_labels, aes(x = x, y = y, label = label), 
  #           color = gamma_labels$color, size = 3, hjust = 0, parse = TRUE, inherit.aes = FALSE) +
...
print(p_gamma)
ggsave("Vary parameters/gamma_distributions_vary_params_scale.pdf", plot = p_gamma, width = 7, height = 7)